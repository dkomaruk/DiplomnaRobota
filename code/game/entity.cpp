...
void RenderEntity(Entity *self, Game *game)
{
    ...
    if(game->shadowPass)
    {
        ShaderSetMatrix4(GetShader(game, "shadow"), "u_model", self->modelMat);
        ShaderSetMatrix4(GetShader(game, "skinned_shadow"), "u_model", self->modelMat);
    }

    if(game->renderAABB)
    {
        ShaderSetVec4(GetShader(game, "line"), "u_color", glm::vec4(0.0f, 1.0f, 0.0f, 1.0f));
        glLineWidth(2.0f);
        ...
        //RenderMesh(game, &self->meshAABB, self->modelMatPosScale, GetShader(game, "line"), 0, self->meshAABB.drawMode);
        RenderMesh(game, &self->meshAABB, self->modelMat, GetShader(game, "line"), 0, self->meshAABB.drawMode);
        glLineWidth(1.0f);
    }
